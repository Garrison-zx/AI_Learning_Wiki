...
> 💥 **本课 Wow Moment**：Attention 的本质不是"让模型关注重要的地方"，而是一种**可微分的动态加权求和**——这意味着"哪里重要"本身是被训练出来的，而不是人工指定的。这个设计让 Transformer 彻底甩掉了 RNN 的串行枷锁。
...
| 术语 | 一句话解释 | 示例 |
|------|-----------|------|
| Token | 模型处理文字的最小单位 | "Hello world" → ["Hello", " world"]，中文每字约 1-2 个 token |
| Embedding | 把 token 变成数字向量 | "苹果" → [0.2, -0.5, 0.8, ...]（768 维） |
| 张量 (Tensor) | 多维数组，神经网络的基本数据结构 | 形状 (2, 5, 768) = 2 个句子，每句 5 个 token，每个 768 维 |
| 权重矩阵 | 神经网络训练的参数 | W_Q, W_K, W_V 都是权重矩阵 |
| softmax | 把一组数变成加和为 1 的概率 | [1, 2, 3] → [0.09, 0.24, 0.67] |
| d_k | Attention 中 Key 向量的维度 | 通常 = d_model / num_heads = 64 |
| 残差连接 | `输出 = 输入 + F(输入)`，跳过中间层 | 解决深网络训练时梯度消失 |
| LayerNorm | 对每个 token 的特征做归一化 | 让激活值保持在合理范围，稳定训练 |
...
| 串行无法并行 | **问题**：计算 h_5 必须等 h_4 完成，h_4 必须等 h_3... **类比**：工厂流水线 vs 并行车间 **量化**：序列长度 512，GPU 利用率不到 10% | [C1] |
...
| LSTM/GRU 的缓解与局限 | LSTM 用门控机制缓解了梯度消失，但**没有解决串行问题** 结论：需要从根本上改变计算范式 | |
...
| 层 | 问题 | 回答 |
|----|------|------|
| **第 1 层** | Attention 是什么？ | 对序列中每个位置，根据与其他位置的相关性做加权求和 |
| **第 2 层** | 为什么需要 Attention？ | RNN 无法并行 + 长程依赖消失，Attention 同时解决两个问题 |
| **第 3 层** | 为什么用 Q×K 点积衡量相似度？ | 点积计算快（矩阵乘法 GPU 高度优化），且保持可微分；对比：加性注意力需要额外参数 |
| **第 4 层** | 为什么除以 √d_k？ | d_k 大时点积方差 = d_k，softmax 会饱和（梯度接近 0）；除以 √d_k 使方差归一 |
| **第 5 层** | 底层数学原理？ | 若 Q, K 各分量 i.i.d. ~ N(0,1)，则 Q·K 的方差 = d_k；除以 √d_k 后方差 = 1 |
| **第 6 层** | 怎么演变来的？往哪里去？ | 加性注意力(2014) → 点积注意力(2015) → 自注意力(2017) → MQA(2019) → GQA(2023) → FlashAttention(2022) → 线性注意力 / Mamba(2023+) |
...
|------|------|------|-----------|------------|
| 诞生 | 正弦位置编码 | 2017 | 固定编码，无需学习 | 不含相对位置，外推差 |
| 改进 | 可学习位置编码（GPT） | 2018 | 自适应学习 | 外推更差 |
| 突破 | RoPE | 2021 | 天然相对位置 + 可外推 | 超长上下文衰减 |
| 修补 | NTK-aware / YaRN | 2023 | 扩展到 128K+ | 工程复杂度增加 |
| 前沿 | 无位置编码探索 | 2024+ | 完全去掉位置编码？ | 实验阶段 |

> 引用：[Su et al., 2021] RoFormer: Enhanced Transformer with Rotary Position Embedding
...
| Causal Mask | Decoder 的自注意力必须遮住未来 token（上三角为 -∞，softmax 后为 0） **原因**：训练时不能偷看答案，推理时也没有未来的词 |
...
- **混合架构**：Transformer 层 + Mamba 层交替，结合两者优势（如 Jamba）
...
**参考答案**：
原论文（2017）用 Post-LN（Attention 之后做归一化）。后来研究发现 Pre-LN（Attention 之前）训练更稳定，梯度可以直接通过残差连接传播，不容易爆炸/消失。GPT 系列、LLaMA、Qwen 等现代大模型都采用 Pre-LN，且通常用 RMSNorm 替代 LayerNorm（去掉均值项，计算更快，效果相当）。
</details>
